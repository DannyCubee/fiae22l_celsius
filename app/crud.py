...
def get_last_reading(db: Session):
    return db.query(models.Readings).order_by(desc(models.Readings.id)).limit(1).first()
...
def get_reading_by_timeframe(db: Session, von: datetime, bis: datetime) -> list:
    return db.query(models.Readings).filter(models.Readings.time >= von, models.Readings.time <= bis).all()
...
# Abfrage der letzten 30 Messungen für das Frontend
